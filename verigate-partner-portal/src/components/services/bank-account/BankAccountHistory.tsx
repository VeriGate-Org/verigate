...
import { generateBankAccountHistory } from "@/lib/mock-services";
import type { BankAccountHistoryItem } from "@/lib/mock-services";
import { config } from "@/lib/config";
import { getBankAccountHistory } from "@/lib/bff-client";
...
const columns: DataGridColumn<BankAccountHistoryItem>[] = [
  {
    id: "outcome",
    header: "Status",
...
export default function BankAccountHistory() {
  const [history, setHistory] = useState<BankAccountHistoryItem[]>([]);
  ...
  const fetchHistory = () => {
    ...
    getBankAccountHistory({ limit: 200 })
      ...
      .catch(() => setHistory(generateBankAccountHistory()))
  ...
  const { data, total, totalPages } = processData(history, {
    searchFields: ["accountNumber", "accountHolder"],
  ...
  if (!isLoading && history.length === 0 && !error) {
    return (
      <VerificationEmptyState
        ...
        description="Bank account verifications you perform will appear here."
      />
    );
  }
