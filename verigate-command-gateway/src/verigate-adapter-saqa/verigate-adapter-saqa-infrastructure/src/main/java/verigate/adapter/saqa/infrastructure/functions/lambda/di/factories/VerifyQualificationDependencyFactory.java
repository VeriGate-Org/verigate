...
package verigate.adapter.saqa.infrastructure.functions.lambda.di.factories;
...
import verigate.adapter.saqa.infrastructure.functions.lambda.di.modules.VerifyQualificationServiceModule;
import verigate.verification.cg.domain.commands.incoming.VerifyPartyCommand;

/** Wrapper for core dependencies. Allows for overrides via Guice. */
public class VerifyQualificationDependencyFactory extends DependencyFactory {

  /** Creates a factory with default Guice module. */
  public VerifyQualificationDependencyFactory() {
    super(Guice.createInjector(Stage.PRODUCTION, new VerifyQualificationServiceModule()));
  }

  /** Creates a factory with specified Guice injector. Useful for test purposes. */
  public VerifyQualificationDependencyFactory(Injector injector) {
    super(injector);
  }
  ...
  public CommandHandler<VerifyPartyCommand, Map<String, String>>
      getVerifyQualificationCommandHandler() {
   ...
   */
  public InvalidMessageQueue<SQSMessage> getVerifyQualificationInvalidMessageQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<InvalidMessageQueue<SQSMessage>>() {},
            Names.named("VerifyQualificationInvalidMessageQueue")));
  }
   ...
   */
  public DeadLetterQueue<SQSMessage> getVerifyQualificationDeadLetterQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<DeadLetterQueue<SQSMessage>>() {},
            Names.named("VerifyQualificationDeadLetterMessageQueue")));
  }
}
