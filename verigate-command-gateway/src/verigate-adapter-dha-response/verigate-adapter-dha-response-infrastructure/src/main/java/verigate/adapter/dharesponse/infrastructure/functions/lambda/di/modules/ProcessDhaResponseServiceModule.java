...
public class ProcessDhaResponseServiceModule extends AbstractModule {
    ...
    @Provides
    @Singleton
    DefaultProcessDhaResponseCommandHandler.EmailFetcher provideEmailFetcher(S3Client s3Client) {
        return new S3EmailFetcher(s3Client);
    }

    @Provides
    @Singleton
    DefaultProcessDhaResponseCommandHandler.MimeParser provideMimeParser() {
        return new MimeEmailParser();
    }

    @Provides
    @Singleton
    DefaultProcessDhaResponseCommandHandler.CommandStoreAdapter provideCommandStoreAdapter(
            DynamoDbClient dynamoDbClient) {
        String tableName = System.getenv("VERIFICATION_COMMAND_STORE_DB");
        return new DhaResponseCommandStoreAdapter(dynamoDbClient, tableName);
    }

    @Provides
    @Singleton
    DhaResponseParserService provideAiAnalyzer(BedrockRuntimeClient bedrockClient,
                                                ObjectMapper objectMapper) {
        String modelId = System.getenv("BEDROCK_MODEL_ID");
        return new AiDhaResponseAnalyzer(bedrockClient,
                modelId != null ? modelId : "us.anthropic.claude-sonnet-4-5-20250929-v1:0",
                objectMapper);
    }
    ...
    @Provides
    @Singleton
    DefaultProcessDhaResponseCommandHandler.PartnerNotifier providePartnerNotifier(
            SesClient sesClient, DynamoDbClient dynamoDbClient) {
        String partnerHubTable = System.getenv("VERIGATE_PARTNER_HUB_TABLE");
        String senderEmail = System.getenv("DHA_RESPONSE_SENDER_EMAIL");
        String portalBaseUrl = System.getenv("PORTAL_BASE_URL");
        return new PartnerNotificationSender(sesClient, dynamoDbClient,
                partnerHubTable,
                senderEmail != null ? senderEmail : "dhaverifications@verigate.co.za",
                portalBaseUrl != null ? portalBaseUrl : "https://portal.verigate.co.za");
    }

    @Provides
    @Singleton
    DefaultProcessDhaResponseCommandHandler provideCommandHandler(
            DefaultProcessDhaResponseCommandHandler.EmailFetcher emailFetcher,
            DefaultProcessDhaResponseCommandHandler.MimeParser mimeParser,
            DefaultProcessDhaResponseCommandHandler.CommandStoreAdapter commandStoreAdapter,
            DhaResponseParserService aiAnalyzer,
            DhaResponseReportService reportService,
            DefaultProcessDhaResponseCommandHandler.PartnerNotifier partnerNotifier) {
