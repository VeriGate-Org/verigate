...
import verigate.adapter.creditbureau.infrastructure.config.CreditBureauApiConfiguration;
import verigate.adapter.creditbureau.infrastructure.http.CreditBureauApiAdapter;
import verigate.adapter.creditbureau.infrastructure.http.CreditBureauHttpAdapter;
import verigate.adapter.creditbureau.infrastructure.mappers.CreditBureauDtoMapper;
import verigate.adapter.creditbureau.infrastructure.services.DefaultCreditBureauService;
...
public class ServiceModule extends AbstractModule {
  ...
  protected void configure() {
    ...
    bind(CreditBureauService.class).to(DefaultCreditBureauService.class);
  ...
  @Provides
  @Singleton
  private CreditBureauApiConfiguration provideCreditBureauApiConfiguration(
      Environment environment, Config config) {
    return new CreditBureauApiConfiguration(environment, config);
  }

  @Provides
  @Singleton
  private CreditBureauHttpAdapter provideCreditBureauHttpAdapter(
      Environment environment, ObjectMapper objectMapper) {
    return new CreditBureauHttpAdapter(environment, objectMapper);
  }

  @Provides
  @Singleton
  private CreditBureauApiAdapter provideCreditBureauApiAdapter(
      CreditBureauHttpAdapter httpAdapter) {
    return new CreditBureauApiAdapter(httpAdapter);
  }

  @Provides
  @Singleton
  private CreditBureauDtoMapper provideCreditBureauDtoMapper() {
    return new CreditBureauDtoMapper();
  }

  @Provides
  @Singleton
  private DefaultCreditBureauService provideCreditBureauService(
      CreditBureauApiAdapter apiAdapter, CreditBureauDtoMapper dtoMapper) {
    return new DefaultCreditBureauService(apiAdapter, dtoMapper);
  }

  @Provides
  @Singleton
  private DefaultPerformCreditCheckCommandHandler providePerformCreditCheckCommandHandler(
      CreditBureauService creditBureauService) {
    return new DefaultPerformCreditCheckCommandHandler(creditBureauService);
  }
