...
package verigate.adapter.cipc.infrastructure.functions.lambda.di.factories;
...
import verigate.adapter.cipc.infrastructure.functions.lambda.di.modules.VerifyCompanyDetailsServiceModule;
import verigate.verification.cg.domain.commands.incoming.VerifyPartyCommand;

/** Wrapper for core dependencies. Allows for overrides via Guice. */
public class VerifyCompanyDetailsDependencyFactory extends DependencyFactory {

  /** Creates a factory with default Guice module. */
  public VerifyCompanyDetailsDependencyFactory() {
    super(Guice.createInjector(Stage.PRODUCTION, new VerifyCompanyDetailsServiceModule()));
  }

  /** Creates a factory with specified Guice injector. Useful for test purposes. */
  public VerifyCompanyDetailsDependencyFactory(Injector injector) {
    super(injector);
  }
  ...
  public CommandHandler<VerifyPartyCommand, Map<String, String>>
      getVerifyCompanyDetailsCommandHandler() {
   ...
   */
  public InvalidMessageQueue<SQSMessage> getVerifyCompanyDetailsInvalidMessageQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<InvalidMessageQueue<SQSMessage>>() {},
            Names.named("VerifyCompanyDetailsInvalidMessageQueue")));
  }
   ...
   */
  public DeadLetterQueue<SQSMessage> getVerifyCompanyDetailsDeadLetterQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<DeadLetterQueue<SQSMessage>>() {},
            Names.named("VerifyCompanyDetailsDeadLetterMessageQueue")));
  }
}