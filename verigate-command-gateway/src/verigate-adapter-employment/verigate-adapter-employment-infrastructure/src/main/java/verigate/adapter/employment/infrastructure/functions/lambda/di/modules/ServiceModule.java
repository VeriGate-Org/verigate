...
import verigate.adapter.employment.infrastructure.config.EmploymentApiConfiguration;
import verigate.adapter.employment.infrastructure.http.EmploymentApiAdapter;
import verigate.adapter.employment.infrastructure.http.EmploymentHttpAdapter;
import verigate.adapter.employment.infrastructure.mappers.EmploymentDtoMapper;
import verigate.adapter.employment.infrastructure.services.DefaultEmploymentVerificationService;
...
public class ServiceModule extends AbstractModule {
  ...
  protected void configure() {
    ...
    // Employment Services
    bind(EmploymentVerificationService.class).to(DefaultEmploymentVerificationService.class);
  ...
  @Provides
  @Singleton
  private EmploymentApiConfiguration provideEmploymentApiConfiguration(
      Environment environment, Config config) {
    return new EmploymentApiConfiguration(environment, config);
  }

  @Provides
  @Singleton
  private EmploymentHttpAdapter provideEmploymentHttpAdapter(
      Environment environment, ObjectMapper objectMapper) {
    return new EmploymentHttpAdapter(environment, objectMapper);
  }

  @Provides
  @Singleton
  private EmploymentApiAdapter provideEmploymentApiAdapter(EmploymentHttpAdapter httpAdapter) {
    return new EmploymentApiAdapter(httpAdapter);
  }

  @Provides
  @Singleton
  private EmploymentDtoMapper provideEmploymentDtoMapper() {
    return new EmploymentDtoMapper();
  }

  @Provides
  @Singleton
  private DefaultEmploymentVerificationService provideEmploymentVerificationService(
      EmploymentApiAdapter apiAdapter, EmploymentDtoMapper dtoMapper) {
    return new DefaultEmploymentVerificationService(apiAdapter, dtoMapper);
  }

  @Provides
  @Singleton
  private DefaultVerifyEmploymentCommandHandler provideVerifyEmploymentCommandHandler(
      EmploymentVerificationService employmentVerificationService) {
    return new DefaultVerifyEmploymentCommandHandler(employmentVerificationService);
  }
