...
export default function PropertyHistory() {
  ...
  const [filteredHistory, setFilteredHistory] = useState<PropertyHistoryItem[]>([]);
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      items = items.filter(
        (i) => i.query.toLowerCase().includes(q),
      );
    }
  ...
  const foundCount = history.filter((h) => h.outcome === "FOUND").length;
  ...
  if (history.length === 0) {
    return (
      <VerificationEmptyState icon={Clock} heading="No history" description="Property searches you perform will appear here." />
    );
  }
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-3 flex-wrap">
        ...
        <SummaryChip label="Found" count={foundCount} className="bg-success/10 text-success" />
        ...
        <div className="console-card-header">
          <div className="flex flex-wrap gap-3 items-center w-full">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" placeholder="Search by query or owner name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="aws-input w-full pl-10 pr-4 py-2 text-sm" />
            </div>
