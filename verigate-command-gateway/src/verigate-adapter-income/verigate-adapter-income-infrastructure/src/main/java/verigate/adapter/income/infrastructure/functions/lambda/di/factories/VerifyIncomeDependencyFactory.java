...
package verigate.adapter.income.infrastructure.functions.lambda.di.factories;
...
import verigate.adapter.income.infrastructure.functions.lambda.di.modules.VerifyIncomeServiceModule;
import verigate.verification.cg.domain.commands.incoming.VerifyPartyCommand;

/** Wrapper for core dependencies. Allows for overrides via Guice. */
public class VerifyIncomeDependencyFactory extends DependencyFactory {

  /** Creates a factory with default Guice module. */
  public VerifyIncomeDependencyFactory() {
    super(Guice.createInjector(Stage.PRODUCTION, new VerifyIncomeServiceModule()));
  }

  /** Creates a factory with specified Guice injector. Useful for test purposes. */
  public VerifyIncomeDependencyFactory(Injector injector) {
    super(injector);
  }
   ...
   */
  public InvalidMessageQueue<SQSMessage> getVerifyIncomeInvalidMessageQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<InvalidMessageQueue<SQSMessage>>() {},
            Names.named("VerifyIncomeInvalidMessageQueue")));
  }
   ...
   */
  public DeadLetterQueue<SQSMessage> getVerifyIncomeDeadLetterQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<DeadLetterQueue<SQSMessage>>() {},
            Names.named("VerifyIncomeDeadLetterMessageQueue")));
  }
}
