...
package verigate.adapter.saqa.infrastructure.config;
...
/**
 * Configuration class for the SAQA Qualification Verification API integration. Provides type-safe
 * access to configuration properties with sensible defaults.
 */
public class SaqaApiConfiguration {
   ...
   */
  public SaqaApiConfiguration(Environment environment, Config config) {
    this.environment = environment;
    this.config = config;
  }

  /**
   * Returns the API key used to authenticate against the SAQA API.
   */
  public String getApiKey() {
    return environment.get(EnvironmentConstants.SAQA_API_KEY);
  }

  /**
   * Returns the base URL for the SAQA API.
   */
  public String getBaseUrl() {
    String baseUrl = getValue(EnvironmentConstants.SAQA_API_URL, "saqa.api.base-url");
    return baseUrl != null ? baseUrl : EnvironmentConstants.DEFAULT_SAQA_API_URL;
  }
  ...
  public int getHttpTimeoutSeconds() {
    return getIntValue(
        EnvironmentConstants.SAQA_HTTP_TIMEOUT_SECONDS,
        "saqa.http.timeout-seconds",
  ...
  public int getHttpRetryAttempts() {
    return getIntValue(
        EnvironmentConstants.SAQA_HTTP_RETRY_ATTEMPTS,
        "saqa.http.retry-attempts",
  ...
  public int getHttpRetryDelayMs() {
    return getIntValue(
        EnvironmentConstants.SAQA_HTTP_RETRY_DELAY_MS,
        "saqa.http.retry-delay-ms",
  ...
  public int getRateLimitRps() {
    return getIntValue(
        EnvironmentConstants.SAQA_RATE_LIMIT_RPS,
        "saqa.rate-limit.requests-per-second",
  ...
  public int getRateLimitBurst() {
    return getIntValue(
        EnvironmentConstants.SAQA_RATE_LIMIT_BURST,
        "saqa.rate-limit.burst-capacity",
  ...
  public boolean isRequestLoggingEnabled() {
    return getBooleanValue(
        EnvironmentConstants.SAQA_ENABLE_REQUEST_LOGGING,
        "saqa.logging.request-enabled",
  ...
  public boolean isResponseLoggingEnabled() {
    return getBooleanValue(
        EnvironmentConstants.SAQA_ENABLE_RESPONSE_LOGGING,
        "saqa.logging.response-enabled",
  ...
  public String getLogLevel() {
    String level = getValue(EnvironmentConstants.SAQA_LOG_LEVEL, "saqa.logging.level");
  ...
  public int getHttpConnectionPoolSize() {
    return getIntValue(
        EnvironmentConstants.SAQA_HTTP_TIMEOUT_SECONDS, "saqa.http.connection-pool-size", 5);
  }
  ...
  public void validate() {
    String apiKey = getApiKey();
    if (apiKey == null || apiKey.trim().isEmpty()) {
      throw new IllegalStateException("SAQA API key is required");
    }

    String baseUrl = getBaseUrl();
    if (baseUrl == null || baseUrl.trim().isEmpty()) {
      throw new IllegalStateException("SAQA base URL is required");
    }

    if (!baseUrl.startsWith("http://") && !baseUrl.startsWith("https://")) {
      throw new IllegalStateException("SAQA base URL must start with http or https");
    }
  }
}
