...
import { saveCheckSession } from "@/lib/services/check-session-service";
import type { CheckSessionResult } from "@/lib/types/check-session";
...
interface NewCheckFormProps {
  prefillSubject?: { idNumber: string; firstName: string; lastName: string };
  prefillChecks?: string[];
}

// ── Component ────────────────────────────────────────────────────────

export default function NewCheckForm({ prefillSubject, prefillChecks }: NewCheckFormProps) {
  // Common fields
  ...
  // ── Prefill from history re-run ──────────────────────────────────────

  useEffect(() => {
    if (prefillSubject) {
      setIdNumber(prefillSubject.idNumber);
      setFirstName(prefillSubject.firstName);
      setLastName(prefillSubject.lastName);
    }
    if (prefillChecks) {
      setSelectedChecks(new Set(prefillChecks));
    }
  }, [prefillSubject, prefillChecks]);
  ...
  // ── Save session after ALL_DONE ──────────────────────────────────────

  useEffect(() => {
    if (state.isSubmitting || savedRef.current) return;
    const entries = Object.entries(state.results);
    if (entries.length === 0) return;
    // Only save when all checks are terminal (success or error)
    const allDone = entries.every(([, v]) => v.status === "success" || v.status === "error");
    if (!allDone) return;

    savedRef.current = true;

    const checks: CheckSessionResult[] = entries.map(([checkType, v]) => {
      const def = CHECK_DEFINITIONS[checkType];
      return {
        checkType,
        label: def?.label ?? checkType,
        status: v.status === "success" ? "success" : "error",
        summary: v.status === "success" ? ["Completed"] : [],
        error: v.error,
        data: v.data,
      };
    });
    ...
    saveCheckSession({
      ...
      // Save is best-effort — don't disrupt the UI
    });
  }, [state.isSubmitting, state.results, idNumber, firstName, lastName]);
