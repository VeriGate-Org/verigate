...
package verigate.adapter.dha.infrastructure.functions.lambda.di.factories;
...
import verigate.adapter.dha.infrastructure.functions.lambda.di.modules.VerifyIdentityServiceModule;
import verigate.verification.cg.domain.commands.incoming.VerifyPartyCommand;

/** Wrapper for core dependencies. Allows for overrides via Guice. */
public class VerifyIdentityDependencyFactory extends DependencyFactory {

  /** Creates a factory with default Guice module. */
  public VerifyIdentityDependencyFactory() {
    super(Guice.createInjector(Stage.PRODUCTION, new VerifyIdentityServiceModule()));
  }

  /** Creates a factory with specified Guice injector. Useful for test purposes. */
  public VerifyIdentityDependencyFactory(Injector injector) {
    super(injector);
  }
  ...
  public CommandHandler<VerifyPartyCommand, Map<String, String>>
      getVerifyIdentityCommandHandler() {
   ...
   */
  public InvalidMessageQueue<SQSMessage> getVerifyIdentityInvalidMessageQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<InvalidMessageQueue<SQSMessage>>() {},
            Names.named("VerifyIdentityInvalidMessageQueue")));
  }
   ...
   */
  public DeadLetterQueue<SQSMessage> getVerifyIdentityDeadLetterQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<DeadLetterQueue<SQSMessage>>() {},
            Names.named("VerifyIdentityDeadLetterMessageQueue")));
  }
}
