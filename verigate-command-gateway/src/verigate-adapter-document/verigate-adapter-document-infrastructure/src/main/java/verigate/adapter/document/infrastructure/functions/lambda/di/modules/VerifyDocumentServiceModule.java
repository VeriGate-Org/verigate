 ...
 */
public final class VerifyDocumentServiceModule extends ServiceModule {
   ...
   */
  public VerifyDocumentServiceModule() {
    super();
  }

  @Provides
  @Singleton
  @Named("VerifyDocumentInvalidMessageQueue")
  private InvalidMessageQueue<SQSMessage> provideVerifyDocumentInvalidMessageQueue(
      Environment environment, SqsClient sqsClient) {
    return new SqsLambdaEventRawMessageQueue(
        sqsClient, environment.get("VERIFY_DOCUMENT_IMQ_NAME"));
  }

  @Provides
  @Singleton
  @Named("VerifyDocumentDeadLetterMessageQueue")
  private DeadLetterQueue<SQSMessage> provideVerifyDocumentDeadLetterMessageQueue(
      Environment environment, SqsClient sqsClient) {
    return new SqsLambdaEventRawMessageQueue(
        sqsClient, environment.get("VERIFY_DOCUMENT_DLQ_NAME"));
  }
  ...
  public CommandHandler<VerifyPartyCommand, Map<String, String>>
      provideVerifyDocumentCommandHandler(
          DefaultVerifyDocumentCommandHandler commandHandler,
          Config config,
          InternalTransportJsonSerializer jsonSerializer) {

    return new RetryCommandHandler<VerifyPartyCommand, Map<String, String>>(
        commandHandler, getDefaultRetry(config), "handler-verify-document");
  }
}
