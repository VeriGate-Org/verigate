 ...
 * It extends the ServiceModule class and provides specific bindings for identity verification.
 */
public final class VerifyIdentityServiceModule extends ServiceModule {

  /**
   * Configures the bindings for the DHA adapter. This method is called by the Guice framework to
   * set up the bindings.
   */
  public VerifyIdentityServiceModule() {
    super();
  }

  @Provides
  @Singleton
  @Named("VerifyIdentityInvalidMessageQueue")
  private InvalidMessageQueue<SQSMessage> provideVerifyIdentityInvalidMessageQueue(
      Environment environment, SqsClient sqsClient) {
    return new SqsLambdaEventRawMessageQueue(
        sqsClient, environment.get("VERIFY_IDENTITY_IMQ_NAME"));
  }

  @Provides
  @Singleton
  @Named("VerifyIdentityDeadLetterMessageQueue")
  private DeadLetterQueue<SQSMessage> provideVerifyIdentityDeadLetterMessageQueue(
      Environment environment, SqsClient sqsClient) {
    return new SqsLambdaEventRawMessageQueue(
        sqsClient, environment.get("VERIFY_IDENTITY_DLQ_NAME"));
  }
  ...
  public CommandHandler<VerifyPartyCommand, Map<String, String>>
      provideVerifyIdentityCommandHandler(
          DefaultVerifyIdentityCommandHandler commandHandler,
          Config config,
          InternalTransportJsonSerializer jsonSerializer) {

    return new RetryCommandHandler<VerifyPartyCommand, Map<String, String>>(
        commandHandler, getDefaultRetry(config), "handler-verify-identity");
  }
}
