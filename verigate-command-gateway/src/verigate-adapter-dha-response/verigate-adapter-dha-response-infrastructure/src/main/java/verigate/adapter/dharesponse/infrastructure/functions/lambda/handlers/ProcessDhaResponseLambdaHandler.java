...
public class ProcessDhaResponseLambdaHandler
    ...
    private static final Logger logger = LoggerFactory.getLogger(ProcessDhaResponseLambdaHandler.class);
    private static final ObjectMapper OBJECT_MAPPER = new ObjectMapper();

    private final DefaultProcessDhaResponseCommandHandler commandHandler;

    public ProcessDhaResponseLambdaHandler() {
        this(new ProcessDhaResponseDependencyFactory());
    }

    public ProcessDhaResponseLambdaHandler(ProcessDhaResponseDependencyFactory factory) {
    ...
    public SQSBatchResponse handleRequest(SQSEvent event, Context context) {
        ...
        for (SQSEvent.SQSMessage message : event.getRecords()) {
            try {
                ...
                JsonNode sqsBody = OBJECT_MAPPER.readTree(message.getBody());
                JsonNode records = sqsBody.get("Records");

                if (records == null || !records.isArray() || records.isEmpty()) {
                    logger.warn("No S3 records in SQS message: {}", message.getMessageId());
                    continue;
                }
                ...
                for (JsonNode record : records) {
                    ...
                    if (bucketName.isBlank() || objectKey.isBlank()) {
                        logger.warn("Invalid S3 event: bucket={}, key={}", bucketName, objectKey);
                        continue;
                    }

                    // URL-decode the object key (S3 event notifications encode spaces as +)
            ...
            } catch (DefaultProcessDhaResponseCommandHandler.PermanentProcessingException e) {
                logger.error("Permanent failure processing SQS message {}: {}",
                        message.getMessageId(), e.getMessage());
                failures.add(new SQSBatchResponse.BatchItemFailure(message.getMessageId()));
            } catch (DefaultProcessDhaResponseCommandHandler.TransientProcessingException e) {
                logger.warn("Transient failure processing SQS message {}: {}",
                        message.getMessageId(), e.getMessage());
                failures.add(new SQSBatchResponse.BatchItemFailure(message.getMessageId()));
            } catch (Exception e) {
                logger.error("Unexpected error processing SQS message {}", message.getMessageId(), e);
                failures.add(new SQSBatchResponse.BatchItemFailure(message.getMessageId()));
            }
        }
