...
export default function NegativeNewsHistory() {
  ...
  const [filteredHistory, setFilteredHistory] = useState<NegativeNewsHistoryItem[]>([]);
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      items = items.filter(
        (i) => i.subjectName.toLowerCase().includes(q),
      );
    }
  ...
  const matchesFoundCount = history.filter((h) => h.outcome === "MATCHES_FOUND").length;
  ...
  const outcomeBadge = (outcome: string) => {
    switch (outcome) {
      ...
      case "MATCHES_FOUND":
        return <OutcomeBadge label="Matches Found" type="warning" />;
      case "FAILED":
  ...
  return (
    <div className="space-y-4">
      <div className="flex gap-3 flex-wrap">
        ...
        <SummaryChip label="Matches Found" count={matchesFoundCount} className="bg-warning/10 text-warning" />
        ...
        <div className="console-card-header">
          <div className="flex flex-wrap gap-3 items-center w-full">
            <div className="relative flex-1 min-w-[200px]">
              ...
              <input type="text" placeholder="Search by name..." value={searchQuery} onChange={(e) => setSearchQuery(e.target.value)} className="aws-input w-full pl-10 pr-4 py-2 text-sm" />
            </div>
            <div className="flex items-center gap-2">
              ...
              <select value={outcomeFilter} onChange={(e) => setOutcomeFilter(e.target.value)} className="aws-select text-sm">
                ...
                <option value="MATCHES_FOUND">Matches Found</option>
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-base-200/50">
                ...
                <th className="text-left px-4 py-2.5 text-xs font-medium text-text-muted uppercase tracking-wide">Entity Type</th>
                <th className="text-left px-4 py-2.5 text-xs font-medium text-text-muted uppercase tracking-wide">Matches</th>
            ...
            <tbody className="divide-y divide-border">
              {paged.map((item) => (
                <tr key={item.verificationId} className="hover:bg-hover/50">
                  <td className="px-4 py-2.5 text-text">{item.subjectName}</td>
                  <td className="px-4 py-2.5 text-text">{item.entityType}</td>
                  <td className="px-4 py-2.5 text-text">{item.matchCount}</td>
              ...
              {paged.length === 0 && (
                <tr><td colSpan={5} className="px-4 py-8 text-center text-text-muted">No results match the current filters.</td></tr>
              )}
            </tbody>
          </table>
        </div>
