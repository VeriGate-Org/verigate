...
import verigate.adapter.cipc.infrastructure.config.CipcApiConfiguration;
import verigate.adapter.cipc.infrastructure.http.CipcCompanyApiAdapter;
import verigate.adapter.cipc.infrastructure.http.CipcHttpAdapter;
import verigate.adapter.cipc.infrastructure.mappers.CipcDtoMapper;
import verigate.adapter.cipc.infrastructure.services.DefaultCipcCompanyService;
...
public class ServiceModule extends AbstractModule {
  ...
  protected void configure() {
    ...
    // CIPC Services
    bind(CipcCompanyService.class).to(DefaultCipcCompanyService.class);
  ...
  @Provides
  @Singleton
  private CipcApiConfiguration provideCipcApiConfiguration(
      Environment environment, Config config) {
    return new CipcApiConfiguration(environment, config);
  }

  @Provides
  @Singleton
  private CipcHttpAdapter provideCipcHttpAdapter(
      Environment environment, ObjectMapper objectMapper) {
    return new CipcHttpAdapter(environment, objectMapper);
  }

  @Provides
  @Singleton
  private CipcCompanyApiAdapter provideCipcCompanyApiAdapter(CipcHttpAdapter httpAdapter) {
    return new CipcCompanyApiAdapter(httpAdapter);
  }

  @Provides
  @Singleton
  private CipcDtoMapper provideCipcDtoMapper() {
    return new CipcDtoMapper();
  }

  @Provides
  @Singleton
  private DefaultCipcCompanyService provideCipcCompanyService(
      CipcCompanyApiAdapter apiAdapter, CipcDtoMapper dtoMapper) {
    return new DefaultCipcCompanyService(apiAdapter, dtoMapper);
  }

  @Provides
  @Singleton
  private DefaultVerifyCompanyDetailsCommandHandler provideVerifyCompanyDetailsCommandHandler(
