 ...
 * <li>Run: mvn test -Dtest=OpenSanctionsLiveIntegrationTest</li>
 * </ol>
 *
...
class OpenSanctionsLiveIntegrationTest {

  private static final Logger LOGGER =
      LoggerFactory.getLogger(OpenSanctionsLiveIntegrationTest.class);

  private static Properties testConfig;
  private static DefaultOpenSanctionsMatchingService matchingService;
  ...
  static void setupLiveTest() throws IOException {
    ...
    boolean configEnabled =
        Boolean.parseBoolean(testConfig.getProperty("integration.test.enabled", "false"));
    boolean runtimeEnabled =
        Boolean.parseBoolean(System.getProperty("opensanctions.it.enabled", "false"))
            || Boolean.parseBoolean(
                System.getenv().getOrDefault("OPENSANCTIONS_IT_ENABLED", "false"));

    // Skip test unless both configuration and explicit runtime flag are enabled
    assumeTrue(
        configEnabled && runtimeEnabled,
        "OpenSanctions live tests are disabled. Enable by setting integration.test.enabled=true "
            + "and -Dopensanctions.it.enabled=true (or OPENSANCTIONS_IT_ENABLED=true).");

    // Validate API key is configured
    String apiKey = testConfig.getProperty("OPENSANCTIONS_API_KEY");
    assumeTrue(
        apiKey != null && !apiKey.contains("your-") && !apiKey.trim().isEmpty(),
        "Please set a valid OpenSanctions API key before running live tests");
    ...
    matchingService = new DefaultOpenSanctionsMatchingService(apiAdapter);
  ...
  private static Properties loadTestConfiguration() throws IOException {
    ...
    try (InputStream baseConfig =
        OpenSanctionsLiveIntegrationTest.class.getResourceAsStream(
    ...
    try (InputStream localConfig =
        OpenSanctionsLiveIntegrationTest.class.getResourceAsStream(
            "/integration-test-local.properties")) {
      if (localConfig != null) {
  ...
  void test01_HealthCheck() throws Exception {
    ...
    EntityMatchRequest healthCheckRequest =
  ...
  void test02_MatchKnownSanctionedPerson() throws Exception {
        ...
        new EntityExample.Builder()
            ...
            .properties(
                Map.of(
                    ...
                    "birthDate",
                        List.of(testConfig.getProperty("test.person.birth.date", "1952-10-07")),
                    "nationality",
                        List.of(testConfig.getProperty("test.person.nationality", "Russian"))))
            .build();
    ...
    assertFalse(
        response.getResponses().isEmpty(), "Should find matches for known sanctioned person");

    // Check for high-confidence matches
    boolean hasHighConfidenceMatch =
    ...
    assertTrue(
        hasHighConfidenceMatch, "Should have at least one high-confidence match (score > 0.8)");
    ...
    response.getResponses().values().stream()
        ...
        .forEach(
            match ->
                LOGGER.info("🎯 High match: {} (score: {})", match.getCaption(), match.getScore()));
  }
        ...
        new EntityExample.Builder()
            ...
            .properties(
                Map.of(
                    ...
                    "jurisdiction",
                        List.of(testConfig.getProperty("test.company.jurisdiction", "RU"))))
            .build();
    ...
    if (hasMatches) {
      ...
      response.getResponses().values().stream()
          ...
          .forEach(
              match ->
                  LOGGER.info(
                      "🏢 Company match: {} (score: {})", match.getCaption(), match.getScore()));
    } else {
      LOGGER.info(
          "ℹ️  No matches found for test company - this may be expected depending on dataset");
    }
  }
  ...
  void test05_HandleInvalidApiKey() throws Exception {
    ...
    DefaultOpenSanctionsMatchingService badService =
        new DefaultOpenSanctionsMatchingService(badApiAdapter);

    EntityExample testExample =
        new EntityExample.Builder()
            .id("invalid-test")
            .schema(DomainConstants.PERSON_SCHEMA)
  ...
  void test07_PerformanceTest() throws Exception {
    ...
    int timeoutMs =
        Integer.parseInt(testConfig.getProperty("integration.test.timeout.seconds", "30")) * 1000;
    assertTrue(
