...
public class QuickLiveTest {
    ...
    public static void main(String[] args) {
        ...
        try {
            ...
            DefaultDeedsWebMatchingService service = new DefaultDeedsWebMatchingService(apiAdapter);
            
            System.out.println("✅ Service initialized");
            
            // Test 1: Health check with clean person
            System.out.println("\n🏥 Testing API connectivity...");
            EntityExample cleanExample = new EntityExample.Builder()
                    .id("clean-test")
            ...
            EntityMatchRequest cleanRequest = new EntityMatchRequest.Builder()
                    .dataset("default")
            ...
            int cleanResults = cleanResponse.getResponses().values().stream()
            ...
            EntityExample sanctionedExample = new EntityExample.Builder()
                    .id("sanctioned-test")
                    .schema(DomainConstants.PERSON_SCHEMA)
                    .properties(Map.of(
            ...
            EntityMatchRequest sanctionsRequest = new EntityMatchRequest.Builder()
                    .dataset("default")
            ...
            int sanctionsResults = sanctionsResponse.getResponses().values().stream()
                    .mapToInt(entityMatches -> entityMatches.getResults().size())
                    .sum();
            System.out.println("✅ Sanctions test completed - found " + sanctionsResults + " results");
            
            // Show high-confidence matches
            long highConfidenceMatches = sanctionsResponse.getResponses().values().stream()
    ...
    private static String getApiKey() {
        ...
        try {
            ...
            try (InputStream is = QuickLiveTest.class.getResourceAsStream("/integration-test-local.properties")) {
                if (is != null) {
                    props.load(is);
            ...
            try (InputStream is = QuickLiveTest.class.getResourceAsStream("/integration-test.properties")) {
                if (is != null) {
                    props.load(is);
                    return props.getProperty("opensanctions.api.key");
                }
