...
package verigate.adapter.saqa.infrastructure.functions.lambda.di.modules;
...
import verigate.adapter.saqa.application.handlers.DefaultVerifyQualificationCommandHandler;
import verigate.adapter.saqa.domain.services.QualificationVerificationService;
import verigate.adapter.saqa.infrastructure.config.SaqaApiConfiguration;
import verigate.adapter.saqa.infrastructure.http.SaqaApiAdapter;
import verigate.adapter.saqa.infrastructure.http.SaqaHttpAdapter;
import verigate.adapter.saqa.infrastructure.mappers.SaqaDtoMapper;
import verigate.adapter.saqa.infrastructure.services.DefaultQualificationVerificationService;
...
/**
 * This class is responsible for configuring the dependency injection bindings for the SAQA adapter.
 * It extends the AbstractModule class from the Google Guice library.
 */
public class ServiceModule extends AbstractModule {

  /**
   * Configures the bindings for the SAQA adapter. This method is called by the Guice framework to
   * set up the bindings.
   */
  @Override
  protected void configure() {
    ...
    // SAQA Services
    bind(QualificationVerificationService.class)
        .to(DefaultQualificationVerificationService.class);
  ...
  @Provides
  @Singleton
  private SaqaApiConfiguration provideSaqaApiConfiguration(
      Environment environment, Config config) {
    return new SaqaApiConfiguration(environment, config);
  }

  @Provides
  @Singleton
  private SaqaHttpAdapter provideSaqaHttpAdapter(
      Environment environment, ObjectMapper objectMapper) {
    return new SaqaHttpAdapter(environment, objectMapper);
  }

  @Provides
  @Singleton
  private SaqaApiAdapter provideSaqaApiAdapter(SaqaHttpAdapter httpAdapter) {
    return new SaqaApiAdapter(httpAdapter);
  }

  @Provides
  @Singleton
  private SaqaDtoMapper provideSaqaDtoMapper() {
    return new SaqaDtoMapper();
  }

  @Provides
  @Singleton
  private DefaultQualificationVerificationService provideQualificationVerificationService(
      SaqaApiAdapter apiAdapter, SaqaDtoMapper dtoMapper) {
    return new DefaultQualificationVerificationService(apiAdapter, dtoMapper);
  }

  @Provides
  @Singleton
  private DefaultVerifyQualificationCommandHandler provideVerifyQualificationCommandHandler(
      QualificationVerificationService qualificationVerificationService) {
    return new DefaultVerifyQualificationCommandHandler(qualificationVerificationService);
  }
