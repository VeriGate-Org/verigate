...
public class MimeEmailParser implements DefaultProcessDhaResponseCommandHandler.MimeParser {

    private static final Logger logger = LoggerFactory.getLogger(MimeEmailParser.class);

    @Override
    public DefaultProcessDhaResponseCommandHandler.ParsedEmail parse(byte[] rawEmail) {
        try {
            ...
            return new DefaultProcessDhaResponseCommandHandler.ParsedEmail(from, subject, body, inReplyTo);
        } catch (Exception e) {
            throw new DefaultProcessDhaResponseCommandHandler.PermanentProcessingException(
                    "Failed to parse MIME email: " + e.getMessage());
        }
    }

    private String extractTextBody(MimeMessage message) throws Exception {
    ...
    private String extractTextFromMultipart(MimeMultipart multipart) throws Exception {
        ...
        for (int i = 0; i < multipart.getCount(); i++) {
            ...
            if (part.isMimeType("text/plain")) {
                ...
                text.append(extractTextFromMultipart(nestedMultipart));
            }
        }
