...
import verigate.adapter.qlink.infrastructure.functions.lambda.di.modules.VerifyBankAccountDetailsServiceModule;
import verigate.verification.cg.domain.commands.incoming.VerifyPartyCommand;

/** Wrapper for core dependencies. Allows for overrides via Guice. */
public class VerifyBankAccountDetailsDependencyFactory extends DependencyFactory {

  /** Creates a factory with default Guice module. */
  public VerifyBankAccountDetailsDependencyFactory() {
    super(Guice.createInjector(Stage.PRODUCTION, new VerifyBankAccountDetailsServiceModule()));
  }

  /** Creates a factory with specified Guice injector. Useful for test purposes. */
  public VerifyBankAccountDetailsDependencyFactory(Injector injector) {
    super(injector);
  }
  ...
  public CommandHandler<VerifyPartyCommand, Map<String, String>>
      getVerifyBankAccountDetailsCommandHandler() {
   ...
   */
  public InvalidMessageQueue<SQSMessage> getVerifyBankAccountDetailsInvalidMessageQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<InvalidMessageQueue<SQSMessage>>() {},
            Names.named("VerifyBankAccountDetailsInvalidMessageQueue")));
  }
   ...
   */
  public DeadLetterQueue<SQSMessage> getVerifyBankAccountDetailsDeadLetterQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<DeadLetterQueue<SQSMessage>>() {},
            Names.named("VerifyBankAccountDetailsDeadLetterMessageQueue")));
  }
}
