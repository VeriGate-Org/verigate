...
/**
 * Configuration class for the Income Verification API integration. Provides type-safe access to
 * configuration properties with sensible defaults.
 */
public class IncomeApiConfiguration {
   ...
   */
  public IncomeApiConfiguration(Environment environment, Config config) {
    this.environment = environment;
    this.config = config;
  }

  /**
   * Returns the API key used to authenticate against the Income Verification API.
   */
  public String getApiKey() {
    return environment.get(EnvironmentConstants.INCOME_API_KEY);
  }

  /**
   * Returns the base URL for the Income Verification API.
   */
  public String getBaseUrl() {
    String baseUrl = getValue(EnvironmentConstants.INCOME_API_URL, "income.api.base-url");
    return baseUrl != null ? baseUrl : EnvironmentConstants.DEFAULT_INCOME_API_URL;
  }
  ...
  public int getHttpTimeoutSeconds() {
    return getIntValue(
        EnvironmentConstants.INCOME_HTTP_TIMEOUT_SECONDS,
        "income.http.timeout-seconds",
  ...
  public int getHttpRetryAttempts() {
    return getIntValue(
        EnvironmentConstants.INCOME_HTTP_RETRY_ATTEMPTS,
        "income.http.retry-attempts",
  ...
  public int getHttpRetryDelayMs() {
    return getIntValue(
        EnvironmentConstants.INCOME_HTTP_RETRY_DELAY_MS,
        "income.http.retry-delay-ms",
  ...
  public int getRateLimitRps() {
    return getIntValue(
        EnvironmentConstants.INCOME_RATE_LIMIT_RPS,
        "income.rate-limit.requests-per-second",
  ...
  public int getRateLimitBurst() {
    return getIntValue(
        EnvironmentConstants.INCOME_RATE_LIMIT_BURST,
        "income.rate-limit.burst-capacity",
  ...
  public boolean isRequestLoggingEnabled() {
    return getBooleanValue(
        EnvironmentConstants.INCOME_ENABLE_REQUEST_LOGGING,
        "income.logging.request-enabled",
  ...
  public boolean isResponseLoggingEnabled() {
    return getBooleanValue(
        EnvironmentConstants.INCOME_ENABLE_RESPONSE_LOGGING,
        "income.logging.response-enabled",
  ...
  public String getLogLevel() {
    String level = getValue(EnvironmentConstants.INCOME_LOG_LEVEL, "income.logging.level");
  ...
  public int getHttpConnectionPoolSize() {
    return getIntValue(
        EnvironmentConstants.INCOME_HTTP_TIMEOUT_SECONDS,
        "income.http.connection-pool-size",
        5);
  }
  ...
  public void validate() {
    String apiKey = getApiKey();
    if (apiKey == null || apiKey.trim().isEmpty()) {
      throw new IllegalStateException("Income API key is required");
    }

    String baseUrl = getBaseUrl();
    if (baseUrl == null || baseUrl.trim().isEmpty()) {
      throw new IllegalStateException("Income API URL is required");
    }

    if (!baseUrl.startsWith("http://") && !baseUrl.startsWith("https://")) {
      throw new IllegalStateException("Income API URL must start with http or https");
    }
  }
}
