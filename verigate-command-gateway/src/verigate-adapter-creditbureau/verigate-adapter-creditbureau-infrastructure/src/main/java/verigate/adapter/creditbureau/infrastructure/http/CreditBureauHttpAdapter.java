...
/**
 * Base HTTP adapter that encapsulates common logic for calling the Credit Bureau API.
 */
public class CreditBureauHttpAdapter {

  private static final Logger logger = LoggerFactory.getLogger(CreditBureauHttpAdapter.class);
   ...
   */
  public CreditBureauHttpAdapter(Environment environment, ObjectMapper objectMapper) {
  ...
  private <T, R> R executePost(String endpoint, T body, ResponseParser<R> parser)
    ...
    try {
      ...
      logger.debug("Credit Bureau POST {} succeeded", endpoint);
      return result;
    } catch (IOException e) {
      logger.error(
          "Failed to parse Credit Bureau response for endpoint {}: {}",
          endpoint,
          e.getMessage(),
          e);
      throw new PermanentException("Failed to parse Credit Bureau API response", e);
    } catch (InterruptedException e) {
      Thread.currentThread().interrupt();
      throw new TransientException("Credit Bureau HTTP request interrupted", e);
  ...
  private HttpRequest buildHttpRequest(String endpoint, String body) {
    String apiKey = requireValue(EnvironmentConstants.CREDITBUREAU_API_KEY, "CREDITBUREAU_API_KEY");
    String baseUrl = requireValue(
        EnvironmentConstants.CREDITBUREAU_API_URL, "CREDITBUREAU_API_URL");

    return HttpRequest.newBuilder()
        .uri(URI.create(baseUrl + endpoint))
        .timeout(DEFAULT_TIMEOUT)
        .header("Content-Type", "application/json")
        .header(DomainConstants.AUTH_HEADER_API_KEY, apiKey)
  ...
  private String evaluateResponse(String endpoint, HttpResponse<String> response)
    ...
    logger.debug(
        "Credit Bureau response for {} - status: {}, body length: {}",
        endpoint,
        statusCode,
    ...
    switch (statusCode) {
      case 400 ->
          throw permanent(
              "Credit Bureau API bad request", statusCode, responseBody,
              "Credit Bureau API bad request: %s");
      case 401 ->
          throw permanent(
              "Credit Bureau API authentication failed",
              statusCode,
              responseBody,
              "Credit Bureau API authentication failed - check API key");
      case 404 ->
          throw permanent(
              "Credit Bureau API resource not found",
              statusCode,
              responseBody,
              "Resource not found in Credit Bureau");
      case 429 ->
          throw transientError(
              "Credit Bureau API rate limit exceeded", statusCode, responseBody);
      case 500, 502, 503, 504 ->
          throw transientError(
              "Credit Bureau API server error", statusCode, responseBody);
      default ->
          throw permanent(
              "Unexpected Credit Bureau API response",
              statusCode,
              responseBody,
              "Unexpected Credit Bureau API response: " + statusCode);
    }
  }
