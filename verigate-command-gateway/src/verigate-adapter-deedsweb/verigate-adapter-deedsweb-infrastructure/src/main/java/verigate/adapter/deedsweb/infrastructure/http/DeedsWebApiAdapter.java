...
package verigate.adapter.deedsweb.infrastructure.http;
...
public class DeedsWebApiAdapter extends DeedsWebHttpAdapter {
  ...
  private static final Logger LOGGER = Logger.getLogger(DeedsWebApiAdapter.class.getName());

  public DeedsWebApiAdapter(DeedsWebApiConfiguration config) {
    super(config);
  }
  ...
  public EntityMatchResponse matchEntities(EntityMatchRequest request)
    ...
    EntityMatchRequestDto requestDto = DeedsWebDtoMapper.mapToDto(request);
    ...
    EntityMatchResponse response = DeedsWebDtoMapper.mapToDomain(responseDto);
