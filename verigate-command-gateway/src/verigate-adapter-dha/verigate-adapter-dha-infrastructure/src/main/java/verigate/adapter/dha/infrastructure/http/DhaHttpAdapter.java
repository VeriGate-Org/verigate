...
/**
 * Base HTTP adapter that encapsulates common logic for calling the DHA API.
 */
public class DhaHttpAdapter {

  private static final Logger logger = LoggerFactory.getLogger(DhaHttpAdapter.class);
   ...
   */
  public DhaHttpAdapter(Environment environment, ObjectMapper objectMapper) {
  ...
  private <T, R> R executePost(String endpoint, T body, ResponseParser<R> parser)
    ...
    try {
      ...
      logger.debug("DHA POST {} succeeded", endpoint);
      return result;
    } catch (IOException e) {
      logger.error(
          "Failed to parse DHA response for endpoint {}: {}",
          endpoint,
          e.getMessage(),
          e);
      throw new PermanentException("Failed to parse DHA API response", e);
    } catch (InterruptedException e) {
      Thread.currentThread().interrupt();
      throw new TransientException("DHA HTTP request interrupted", e);
  ...
  private HttpRequest buildHttpRequest(String endpoint, String body) {
    String apiKey = requireValue(EnvironmentConstants.DHA_API_KEY, "DHA_API_KEY");
    String baseUrl = requireValue(EnvironmentConstants.DHA_BASE_URL, "DHA_BASE_URL");
  ...
  private String evaluateResponse(String endpoint, HttpResponse<String> response)
    ...
    logger.debug(
        "DHA response for {} - status: {}, body length: {}",
        endpoint,
        statusCode,
    ...
    switch (statusCode) {
      case 400 ->
          throw permanent(
              "DHA API bad request", statusCode, responseBody, "DHA API bad request: %s");
      case 401 ->
          throw permanent(
              "DHA API authentication failed",
              statusCode,
              responseBody,
              "DHA API authentication failed - check API key");
      case 404 ->
          throw permanent(
              "DHA API resource not found",
              statusCode,
              responseBody,
              "Resource not found in DHA");
      case 429 ->
          throw transientError("DHA API rate limit exceeded", statusCode, responseBody);
      case 500, 502, 503, 504 ->
          throw transientError("DHA API server error", statusCode, responseBody);
      default ->
          throw permanent(
              "Unexpected DHA API response",
              statusCode,
              responseBody,
              "Unexpected DHA API response: " + statusCode);
    }
  }
