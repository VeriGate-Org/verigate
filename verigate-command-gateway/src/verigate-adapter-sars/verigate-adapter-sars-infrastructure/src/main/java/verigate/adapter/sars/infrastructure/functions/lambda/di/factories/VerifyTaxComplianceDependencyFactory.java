...
import verigate.adapter.sars.infrastructure.functions.lambda.di.modules.VerifyTaxComplianceServiceModule;
import verigate.verification.cg.domain.commands.incoming.VerifyPartyCommand;

/** Wrapper for core dependencies. Allows for overrides via Guice. */
public class VerifyTaxComplianceDependencyFactory extends DependencyFactory {

  /** Creates a factory with default Guice module. */
  public VerifyTaxComplianceDependencyFactory() {
    super(Guice.createInjector(Stage.PRODUCTION, new VerifyTaxComplianceServiceModule()));
  }

  /** Creates a factory with specified Guice injector. Useful for test purposes. */
  public VerifyTaxComplianceDependencyFactory(Injector injector) {
    super(injector);
  }
  ...
  public CommandHandler<VerifyPartyCommand, Map<String, String>>
      getVerifyTaxComplianceCommandHandler() {
   ...
   */
  public InvalidMessageQueue<SQSMessage> getVerifyTaxComplianceInvalidMessageQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<InvalidMessageQueue<SQSMessage>>() {},
            Names.named("VerifyTaxComplianceInvalidMessageQueue")));
  }
   ...
   */
  public DeadLetterQueue<SQSMessage> getVerifyTaxComplianceDeadLetterQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<DeadLetterQueue<SQSMessage>>() {},
            Names.named("VerifyTaxComplianceDeadLetterMessageQueue")));
  }
}
