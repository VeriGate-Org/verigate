...
/**
 * Base HTTP adapter that encapsulates common logic for calling the Fraud Watchlist API.
 */
public class FraudWatchlistHttpAdapter {

  private static final Logger logger = LoggerFactory.getLogger(FraudWatchlistHttpAdapter.class);
   ...
   */
  public FraudWatchlistHttpAdapter(Environment environment, ObjectMapper objectMapper) {
  ...
  private <T, R> R executePost(String endpoint, T body, ResponseParser<R> parser)
    ...
    try {
      ...
      logger.debug("Fraud Watchlist POST {} succeeded", endpoint);
      return result;
    } catch (IOException e) {
      logger.error(
          "Failed to parse Fraud Watchlist response for endpoint {}: {}",
          endpoint,
          e.getMessage(),
          e);
      throw new PermanentException("Failed to parse Fraud Watchlist API response", e);
    } catch (InterruptedException e) {
      Thread.currentThread().interrupt();
      throw new TransientException("Fraud Watchlist HTTP request interrupted", e);
  ...
  private String evaluateResponse(String endpoint, HttpResponse<String> response)
    ...
    logger.debug(
        "Fraud Watchlist response for {} - status: {}, body length: {}",
        endpoint,
        statusCode,
    ...
    switch (statusCode) {
      case 400 ->
          throw permanent(
              "Fraud Watchlist API bad request", statusCode, responseBody,
              "Fraud Watchlist API bad request: %s");
      case 401 ->
          throw permanent(
              "Fraud Watchlist API authentication failed",
              statusCode,
              responseBody,
              "Fraud Watchlist API authentication failed - check API key");
      case 404 ->
          throw permanent(
              "Fraud Watchlist API resource not found",
              statusCode,
              responseBody,
              "Resource not found in Fraud Watchlist");
      case 429 ->
          throw transientError("Fraud Watchlist API rate limit exceeded", statusCode, responseBody);
      case 500, 502, 503, 504 ->
          throw transientError("Fraud Watchlist API server error", statusCode, responseBody);
      default ->
          throw permanent(
              "Unexpected Fraud Watchlist API response",
              statusCode,
              responseBody,
              "Unexpected Fraud Watchlist API response: " + statusCode);
    }
  }
