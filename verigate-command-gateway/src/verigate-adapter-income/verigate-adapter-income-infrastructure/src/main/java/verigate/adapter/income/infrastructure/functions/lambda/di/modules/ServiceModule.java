...
import verigate.adapter.income.infrastructure.config.IncomeApiConfiguration;
import verigate.adapter.income.infrastructure.http.IncomeHttpAdapter;
...
import verigate.adapter.income.infrastructure.services.DefaultIncomeVerificationService;
...
public class ServiceModule extends AbstractModule {
  ...
  protected void configure() {
    ...
    // Income Services
    bind(IncomeVerificationService.class).to(DefaultIncomeVerificationService.class);
  ...
  @Provides
  @Singleton
  private IncomeApiConfiguration provideIncomeApiConfiguration(
      Environment environment, Config config) {
    return new IncomeApiConfiguration(environment, config);
  }

  @Provides
  @Singleton
  private IncomeHttpAdapter provideIncomeHttpAdapter(
      Environment environment, ObjectMapper objectMapper) {
    return new IncomeHttpAdapter(environment, objectMapper);
  }

  @Provides
  @Singleton
  private IncomeVerificationApiAdapter provideIncomeVerificationApiAdapter(
      IncomeHttpAdapter httpAdapter) {
    return new IncomeVerificationApiAdapter(httpAdapter);
  }

  @Provides
  @Singleton
  private IncomeDtoMapper provideIncomeDtoMapper() {
    return new IncomeDtoMapper();
  }

  @Provides
  @Singleton
  private DefaultIncomeVerificationService provideIncomeVerificationService(
      IncomeVerificationApiAdapter apiAdapter, IncomeDtoMapper dtoMapper) {
    return new DefaultIncomeVerificationService(apiAdapter, dtoMapper);
  }

  @Provides
  @Singleton
  private DefaultVerifyIncomeCommandHandler provideVerifyIncomeCommandHandler(
      IncomeVerificationService incomeVerificationService) {
    return new DefaultVerifyIncomeCommandHandler(incomeVerificationService);
  }
