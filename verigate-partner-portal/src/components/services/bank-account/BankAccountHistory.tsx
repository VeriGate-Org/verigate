...
export default function BankAccountHistory() {
  ...
  const [filteredHistory, setFilteredHistory] = useState<BankAccountHistoryItem[]>([]);
  ...
  useEffect(() => {
    ...
    if (searchQuery) {
      ...
      items = items.filter(
        (i) => i.accountNumber.toLowerCase().includes(q) || i.accountHolder.toLowerCase().includes(q),
      );
    }
