...
export default function IdentityVerificationHistory() {
  ...
  const outcomeBadge = (outcome: string) => {
    switch (outcome) {
      ...
      case "NOT_FOUND":
        return <span className="px-2 py-0.5 text-xs rounded-full bg-warning/10 text-warning">Not Found</span>;
      case "DECEASED":
        return <span className="px-2 py-0.5 text-xs rounded-full bg-danger/10 text-danger">Deceased</span>;
      case "FAILED":
