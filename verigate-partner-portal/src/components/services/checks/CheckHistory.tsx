  ...
  if (isLoading) {
    return (
      <div className="flex items-center justify-center py-16">
        ...
        <span className="ml-2 text-sm text-text-muted">Loading check session history...</span>
      </div>
    );
  }
    ...
    return (
      <div className="rounded-lg border border-danger/40 bg-danger/5 p-4 flex items-start gap-3">
        ...
        <div className="flex-1">
          <p className="text-sm font-medium text-danger">Failed to load check session history</p>
  ...
  if (sessions.length === 0) {
    return (
      <VerificationEmptyState
        icon={Clock}
        heading="No check sessions yet"
        description="Check sessions you run will appear here."
      />
    );
  }
