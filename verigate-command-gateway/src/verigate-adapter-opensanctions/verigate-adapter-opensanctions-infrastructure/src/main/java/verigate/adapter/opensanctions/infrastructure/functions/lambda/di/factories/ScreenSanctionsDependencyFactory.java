...
package verigate.adapter.opensanctions.infrastructure.functions.lambda.di.factories;
...
import verigate.adapter.opensanctions.infrastructure.functions.lambda.di.modules.ScreenSanctionsServiceModule;
import verigate.verification.cg.domain.commands.incoming.VerifyPartyCommand;

/** Wrapper for core dependencies. Allows for overrides via Guice. */
public class ScreenSanctionsDependencyFactory extends DependencyFactory {

  /** Creates a factory with default Guice module. */
  public ScreenSanctionsDependencyFactory() {
    super(Guice.createInjector(Stage.PRODUCTION, new ScreenSanctionsServiceModule()));
  }

  /** Creates a factory with specified Guice injector. Useful for test purposes. */
  public ScreenSanctionsDependencyFactory(Injector injector) {
    super(injector);
  }
  ...
  public CommandHandler<VerifyPartyCommand, Map<String, String>>
      getScreenSanctionsCommandHandler() {
   ...
   */
  public InvalidMessageQueue<SQSMessage> getScreenSanctionsInvalidMessageQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<InvalidMessageQueue<SQSMessage>>() {},
            Names.named("ScreenSanctionsInvalidMessageQueue")));
  }
   ...
   */
  public DeadLetterQueue<SQSMessage> getScreenSanctionsDeadLetterQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<DeadLetterQueue<SQSMessage>>() {},
            Names.named("ScreenSanctionsDeadLetterMessageQueue")));
  }
}
