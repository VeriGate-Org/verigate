...
/**
 * Base HTTP adapter that encapsulates common logic for calling the Document Verification API.
 */
public class DocumentHttpAdapter {

  private static final Logger logger = LoggerFactory.getLogger(DocumentHttpAdapter.class);
   ...
   */
  public DocumentHttpAdapter(Environment environment, ObjectMapper objectMapper) {
  ...
  private <T, R> R executePost(String endpoint, T body, ResponseParser<R> parser)
    ...
    try {
      ...
      logger.debug("Document API POST {} succeeded", endpoint);
      return result;
    } catch (IOException e) {
      logger.error(
          "Failed to parse Document API response for endpoint {}: {}",
          endpoint,
          e.getMessage(),
          e);
      throw new PermanentException("Failed to parse Document Verification API response", e);
    } catch (InterruptedException e) {
      Thread.currentThread().interrupt();
      throw new TransientException("Document API HTTP request interrupted", e);
  ...
  private HttpRequest buildHttpRequest(String endpoint, String body) {
    String baseUrl = requireValue(EnvironmentConstants.DOCUMENT_API_URL, "DOCUMENT_API_URL");
  ...
  private String evaluateResponse(String endpoint, HttpResponse<String> response)
    ...
    logger.debug(
        "Document API response for {} - status: {}, body length: {}",
        endpoint,
        statusCode,
    ...
    switch (statusCode) {
      case 400 ->
          throw permanent(
              "Document API bad request", statusCode, responseBody,
              "Document API bad request: %s");
      case 401 ->
          throw permanent(
              "Document API authentication failed",
              statusCode,
              responseBody,
              "Document API authentication failed - check credentials");
      case 404 ->
          throw permanent(
              "Document API resource not found",
              statusCode,
              responseBody,
              "Resource not found in Document Verification API");
      case 429 ->
          throw transientError("Document API rate limit exceeded", statusCode, responseBody);
      case 500, 502, 503, 504 ->
          throw transientError("Document API server error", statusCode, responseBody);
      default ->
          throw permanent(
              "Unexpected Document API response",
              statusCode,
              responseBody,
              "Unexpected Document API response: " + statusCode);
    }
  }
