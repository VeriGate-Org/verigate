...
/**
 * Configuration class for the DHA Identity Verification API integration. Provides type-safe access
 * to configuration properties with sensible defaults.
 */
public class DhaApiConfiguration {
   ...
   */
  public DhaApiConfiguration(Environment environment, Config config) {
    this.environment = environment;
    this.config = config;
  }

  /**
   * Returns the API key used to authenticate against the DHA API.
   */
  public String getApiKey() {
    return environment.get(EnvironmentConstants.DHA_API_KEY);
  }

  /**
   * Returns the base URL for the DHA API.
   */
  public String getBaseUrl() {
    String baseUrl = getValue(EnvironmentConstants.DHA_BASE_URL, "dha.api.base-url");
    return baseUrl != null ? baseUrl : EnvironmentConstants.DEFAULT_DHA_BASE_URL;
  }
  ...
  public int getHttpTimeoutSeconds() {
    return getIntValue(
        EnvironmentConstants.DHA_HTTP_TIMEOUT_SECONDS,
        "dha.http.timeout-seconds",
  ...
  public int getHttpRetryAttempts() {
    return getIntValue(
        EnvironmentConstants.DHA_HTTP_RETRY_ATTEMPTS,
        "dha.http.retry-attempts",
  ...
  public int getHttpRetryDelayMs() {
    return getIntValue(
        EnvironmentConstants.DHA_HTTP_RETRY_DELAY_MS,
        "dha.http.retry-delay-ms",
  ...
  public int getRateLimitRps() {
    return getIntValue(
        EnvironmentConstants.DHA_RATE_LIMIT_RPS,
        "dha.rate-limit.requests-per-second",
  ...
  public int getRateLimitBurst() {
    return getIntValue(
        EnvironmentConstants.DHA_RATE_LIMIT_BURST,
        "dha.rate-limit.burst-capacity",
  ...
  public boolean isRequestLoggingEnabled() {
    return getBooleanValue(
        EnvironmentConstants.DHA_ENABLE_REQUEST_LOGGING,
        "dha.logging.request-enabled",
  ...
  public boolean isResponseLoggingEnabled() {
    return getBooleanValue(
        EnvironmentConstants.DHA_ENABLE_RESPONSE_LOGGING,
        "dha.logging.response-enabled",
  ...
  public String getLogLevel() {
    String level = getValue(EnvironmentConstants.DHA_LOG_LEVEL, "dha.logging.level");
  ...
  public int getHttpConnectionPoolSize() {
    return getIntValue(
        EnvironmentConstants.DHA_HTTP_TIMEOUT_SECONDS, "dha.http.connection-pool-size", 5);
  }
  ...
  public void validate() {
    String apiKey = getApiKey();
    if (apiKey == null || apiKey.trim().isEmpty()) {
      throw new IllegalStateException("DHA API key is required");
    }

    String baseUrl = getBaseUrl();
    if (baseUrl == null || baseUrl.trim().isEmpty()) {
      throw new IllegalStateException("DHA base URL is required");
    }

    if (!baseUrl.startsWith("http://") && !baseUrl.startsWith("https://")) {
      throw new IllegalStateException("DHA base URL must start with http or https");
    }
  }
}
