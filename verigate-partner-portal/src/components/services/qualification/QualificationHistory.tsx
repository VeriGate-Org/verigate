...
import { generateQualificationHistory } from "@/lib/mock-services";
import type { QualificationHistoryItem } from "@/lib/mock-services";
import { config } from "@/lib/config";
import { getQualificationHistory } from "@/lib/bff-client";
...
export default function QualificationHistory() {
  const [history, setHistory] = useState<QualificationHistoryItem[]>([]);
  const [filteredHistory, setFilteredHistory] = useState<QualificationHistoryItem[]>([]);
  ...
  if (history.length === 0) {
    return (
      <VerificationEmptyState icon={Clock} heading="No history" description="Qualification verifications you perform will appear here." />
    );
  }
          ...
          <table className="w-full text-sm">
            <thead>
              <tr className="border-b border-border bg-base-200/50">
                ...
                <th className="text-left px-4 py-2.5 text-xs font-medium text-text-muted uppercase tracking-wide">Institution</th>
            ...
            <tbody className="divide-y divide-border">
              {paged.map((item) => (
                <tr key={item.verificationId} className="hover:bg-hover/50">
                  ...
                  <td className="px-4 py-2.5 text-text">{item.institution}</td>
                  <td className="px-4 py-2.5 text-text-muted">{new Date(item.verifiedAt).toLocaleDateString()}</td>
                </tr>
              ))}
              {paged.length === 0 && (
