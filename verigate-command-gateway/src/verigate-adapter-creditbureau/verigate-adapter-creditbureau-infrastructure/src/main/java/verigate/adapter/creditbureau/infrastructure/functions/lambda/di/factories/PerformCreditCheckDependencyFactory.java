...
package verigate.adapter.creditbureau.infrastructure.functions.lambda.di.factories;
...
import verigate.adapter.creditbureau.infrastructure.functions.lambda.di.modules.PerformCreditCheckServiceModule;
import verigate.verification.cg.domain.commands.incoming.VerifyPartyCommand;

/** Wrapper for core dependencies. Allows for overrides via Guice. */
public class PerformCreditCheckDependencyFactory extends DependencyFactory {

  /** Creates a factory with default Guice module. */
  public PerformCreditCheckDependencyFactory() {
    super(Guice.createInjector(Stage.PRODUCTION, new PerformCreditCheckServiceModule()));
  }

  /** Creates a factory with specified Guice injector. Useful for test purposes. */
  public PerformCreditCheckDependencyFactory(Injector injector) {
    super(injector);
  }
  ...
  public CommandHandler<VerifyPartyCommand, Map<String, String>>
      getPerformCreditCheckCommandHandler() {
   ...
   */
  public InvalidMessageQueue<SQSMessage> getPerformCreditCheckInvalidMessageQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<InvalidMessageQueue<SQSMessage>>() {},
            Names.named("PerformCreditCheckInvalidMessageQueue")));
  }
   ...
   */
  public DeadLetterQueue<SQSMessage> getPerformCreditCheckDeadLetterQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<DeadLetterQueue<SQSMessage>>() {},
            Names.named("PerformCreditCheckDeadLetterMessageQueue")));
  }
}
