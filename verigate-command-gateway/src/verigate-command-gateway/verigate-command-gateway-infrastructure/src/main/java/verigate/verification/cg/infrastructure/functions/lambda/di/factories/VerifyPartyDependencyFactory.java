...
package verigate.verification.cg.infrastructure.functions.lambda.di.factories;
...
import verigate.verification.cg.infrastructure.functions.lambda.di.modules.VerifyPartyServiceModule;

/** Wrapper for core dependencies. Allows for overrides via Guice. */
public class VerifyPartyDependencyFactory extends DependencyFactory {

  /** Creates a factory with default Guice module. */
  public VerifyPartyDependencyFactory() {
    super(Guice.createInjector(Stage.PRODUCTION, new VerifyPartyServiceModule()));
  }

  /** Creates a factory with specified Guice injector. Useful for test purposes. */
  public VerifyPartyDependencyFactory(Injector injector) {
    super(injector);
  }
   ...
   */
  public CommandHandler<VerifyPartyCommand, Map<String, String>> getVerifyPartyCommandHandler() {
   ...
   */
  public InvalidMessageQueue<SQSMessage> getVerifyPartyInvalidMessageQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<InvalidMessageQueue<SQSMessage>>() {},
            Names.named("VerifyPartyInvalidMessageQueue")));
  }
   ...
   */
  public DeadLetterQueue<SQSMessage> getVerifyPartyDeadLetterQueue() {
    return injector.getInstance(
        Key.get(
            new TypeLiteral<DeadLetterQueue<SQSMessage>>() {},
            Names.named("VerifyPartyDeadLetterMessageQueue")));
  }
}
