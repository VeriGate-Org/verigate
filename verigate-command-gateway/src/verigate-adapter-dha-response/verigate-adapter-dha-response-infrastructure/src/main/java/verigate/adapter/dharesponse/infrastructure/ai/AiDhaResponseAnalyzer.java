...
public class AiDhaResponseAnalyzer implements DhaResponseParserService {
    ...
    private static final Logger logger = LoggerFactory.getLogger(AiDhaResponseAnalyzer.class);
    ...
    public AiDhaResponseAnalyzer(BedrockRuntimeClient bedrockClient, String modelId,
    ...
    @Override
    public DhaResponseVerificationResult analyzeResponse(String emailBody, String permitNumber,
        ...
        try {
            ...
            String contentText = responseJson.path("content").get(0).path("text").asText();

            // Parse the JSON from the response (may be wrapped in markdown code block)
    ...
    private DhaResponseVerificationResult mapToResult(JsonNode json) {
        ...
        return new DhaResponseVerificationResult(
                outcome,
                null, null,
                json.path("holderName").isNull() ? null : json.path("holderName").asText(),
                json.path("nationality").isNull() ? null : json.path("nationality").asText(),
                json.path("isAuthentic").asBoolean(false),
                json.path("isCurrentlyValid").asBoolean(false),
                json.path("expiryDate").isNull() ? null : json.path("expiryDate").asText(),
                json.path("employerMatch").isNull() ? null : json.path("employerMatch").asText(),
    ...
    private String loadPromptTemplate() {
        try (InputStream is = getClass().getResourceAsStream("/prompts/dha-response-analysis.txt")) {
            if (is == null) {
                throw new IllegalStateException("Prompt template not found: /prompts/dha-response-analysis.txt");
            }
